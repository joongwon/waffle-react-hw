...
} & Omit<InputHTMLAttributes<HTMLInputElement>, "onFocus" | "onChange" | "onBlur">;
...
export default function SnackNameInput({ onChange, ...inputProps }: Props) {
  ...
  return (
    <div className="snack-name">
      <input {...inputProps} onChange={(e) => onChange(e.target.value)} 
        onFocus={() => setIsComplOpen(true)}
        onBlur={() => setTimeout(() => setIsComplOpen(false), 200)}
        />
      {isComplOpen && (snacks.length > 0 ? (
